...
class Sorts:
    ...
    def radix_sort(a):
        ...
        -------------------------------------------------------
        """
        passes = floor(log10(max(a) + 1))
        temp = [[], [], [], [], [], [], [], [], [], []]

        for digit in range(passes + 1):
            pos = (10 ** digit) * 10
            for element in a:
                new_pos = element % pos // (10 ** digit)
                temp[new_pos].append(element)
            index = 0
            for temp_elem in temp:
                while temp_elem != []:
                    a[index] = temp_elem.pop(0)
                    index += 1
